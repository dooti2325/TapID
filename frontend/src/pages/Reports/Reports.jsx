...
function Reports() {
  ...
  const exportCSV = () => {
    ...
    const csvContent = [
      headers.join(','),
      ...reports.records.map(r => `"${new Date(r.time).toLocaleString()}","${r.name}","${r.enrollment_number}","${r.branch}","${r.status}"`)
    ].join('\n');
            ...
            {loading ? (
              ...
              reports.records.map((record, idx) => (
                <tr key={idx}>
                  ...
                  <td className="font-medium text-gray-100">{record.name}</td>
                  <td className="enrollment">{record.enrollment_number}</td>
                  <td className="text-gray-400 text-sm">{record.branch}</td>
                  <td>
